...
function HeaderTabView({ autoWidthTabBar, renderScene, navigationState, hideTabBar = false, renderTabBar: renderTabBarProps, ...props }) {
    ...
    const renderTabBar = useCallback((props) => {
        ...
        return (<>
          {autoWidthTabBar ? (<ScollableAutoWidthTabBar {...props}/>) : (<ScollableTabBar {...props}/>)}
        </>);
    ...
    const _renderScene = (props) => {
        const focused = navigationState.index === props.route?.index || Platform.OS !== "web";
        return (<>
        <View style={{ flex: 1, display: focused ? "flex" : "none" }}>
          {renderScene(props)}
        </View>
        {Platform.OS === "web" && (<TabSpinner index={props.route?.index} style={{ display: focused ? "none" : "flex" }}/>)}
      </>);
    };
    return (<CollapsibleTabView renderTabBar={renderTabBar} lazy onPullEnough={onPullEnough} minHeaderHeight={insets.top + StatusBarHeight} refreshControlColor={isDark ? colors.gray[400] : colors.gray[700]} refreshHeight={60} renderScene={_renderScene} navigationState={navigationState} renderRefreshControl={RefreshControl} {...props}/>);
}
