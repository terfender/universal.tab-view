...
const TabScene = ({ route }) => {
    return (<TabInfiniteScrollList style={{ backgroundColor: "#333" }} index={route.index} data={new Array(LIST_LENGTH).fill(0)} renderItem={({ index }) => {
            return (<View style={{
                    height: ITEM_HEIGHT,
                    backgroundColor: "#fff",
                    marginBottom: 8,
        ...
        }} estimatedItemSize={ITEM_HEIGHT} overscan={{
            main: ITEM_HEIGHT,
            reverse: ITEM_HEIGHT,
        }}/>);
};
export const Basic = () => {
    ...
    return (<HeaderTabView onStartRefresh={onStartRefresh} isRefreshing={isRefreshing} navigationState={{ index, routes }} renderScene={renderScene} onIndexChange={setIndex} renderScrollHeader={renderHeader} minHeaderHeight={44} refreshControlTop={44} autoWidthTabBar animationHeaderPosition={animationHeaderPosition} animationHeaderHeight={animationHeaderHeight} style={Platform.select({
            web: { flex: "none" },
            default: undefined,
        })}/>);
};
//# sourceMappingURL=tab-view.stories.js.map